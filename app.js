    ...
    if (capsMode === true) {
      if (lang === 'ru') {
        if (shift === false) {
          ...
          keyElement.innerHTML = `<span>${keyboardset[i].toUpperCase()}</span>`;
          i += 1;
        } else {
          const keyboardset = ruKeysShift.flat();
          keyElement.innerHTML = `<span>${keyboardset[i].toLowerCase()}</span>`;
          i += 1;
        }
      }
      if (lang === 'en') {
      ...
      if (lang === 'ru') {
        if (shift === false) {
          const keyboardset = ruKeys.flat();
          keyElement.innerHTML = `<span>${keyboardset[i]}</span>`;
          i += 1;
        } else {
          const keyboardset = ruKeysShift.flat();
          keyElement.innerHTML = `<span>${keyboardset[i]}</span>`;
